...
func Summarize(data []float64) Summary {
	...
	variance := sum2 / float64(len(data)-1)
...
// Confidence represents the inverse of the probability that the null hypothesis
// will be rejected by chance.
type Confidence int

func (c Confidence) String() string {
	return fmt.Sprintf("%v%%", studentPct[c])
}

const (
	// C80 is a confidence level of 80%.
	C80 Confidence = iota
	// C90 is a confidence level of 90%.
	C90
	// C95 is a confidence level of 95%.
	C95
	// C98 is a confidence level of 98%.
	C98
	// C99 is a confidence level of 99%.
	C99
	// C995 is a confidence level of 99.5%.
	C995
)

// Compare returns the statistical difference between the two summaries using
// Student's t-test.
func Compare(a, b Summary, confidence Confidence) Difference {
	t := student[0][confidence]
	if i := int(a.N + b.N - 2); i <= nStudent {
		t = student[i][confidence]
	}
...
var (
	studentPct = [nConf]float64{80, 90, 95, 98, 99, 99.5}
	student    = [nStudent + 1][nConf]float64{
